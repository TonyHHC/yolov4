...
#--------------------------------------------------------------------
strParameterFile = 'D:/Project/Tony/Python/yolov4/TX.HA/parameter.xml'
strLabelingXMLTemplate = 'D:/Project/Tony/Python/yolov4/TX.HA/labeling_template.xml'
...
ratio = (0.8, 0.2)
listInterval = ('1d', '60n', '30n', '15n', '5n')
...
def create_config(darkhome, config, configFile, datasFile, classList, batch, subdivisions, width, height, anchors, train_files_count):
	...
	max_batches = len(classList)*2000 if len(classList) >= 3 else 6000
	if train_files_count > max_batches:
		max_batches = int(math.ceil(train_files_count/2000))*2000
...
if __name__ == '__main__':
	...
	for i in listInterval:
		for j in range(2012,2021):
			strTmp = '{}_{}*.png'.format(i, j)
			listTmp = glob.glob(os.path.join(originalFolder, strTmp))
			listImg.extend(listTmp)
	
	test = list()
	for i in listInterval:
		strTmp = '{}_{}*.png'.format(i, 2021)
		listTmp = glob.glob(os.path.join(originalFolder, strTmp))
		test.extend(listTmp)
	...
	indices_for_splitting = [int(len(listImg)*ratio[0])]
	train, val = np.split(listImg, indices_for_splitting)
	train_files_count = len(train)
	...
	create_config(darkhome, config, configFile, datasFile, classList, batch, subdivisions, width, height, anchors, train_files_count)
